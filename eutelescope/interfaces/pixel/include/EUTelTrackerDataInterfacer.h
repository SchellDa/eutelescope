...
namespace eutelescope {
  ...
  protected:
    //!	The vector holding std::reference_wrapper to allow to loop over
    //!EUTelBaseSparsePixel const &
    /*!	In order to write generic processors, we want to instantiate a
     *EUTelTrackerDataInterfacer
     *	with a factory, not caring about the concrete implementation. Such
     *instances can only loop
     *  over the very basic pixel type - EUTelBaseSparsePixel.
     *  Since teh concrete implementation will hold a std::vector of the actual
     *pixel types, we
     *  use a reference to those to loop over them. Since (certain) operations
     *on the actual pixel
     *  vector (in the implementing class) may cause a reallocate, we need to
    ...
    /*! This method will be called whenever an operation on the actual pixel
     *vector might cause a
     *	reallocation. It clears the _refVec and will loop over the pixel vector
     *and insert
  ...
  public:
     ...
     *	dynamic_cast<PixelType const &> will ensure this - and, given it fails,
     *throw an exception.
     *	Using the actual implementing class and
     *	EUTelTrackerDataInterfacerImpl<PixelType>::push_back(PixelType const &
     *pixel) will not have
     ...
     *	virtual and will be overriden by the actual implementation. The return
     *type is a
     *	EUTelBaseSparsePixel const & and the overriden implementation will
